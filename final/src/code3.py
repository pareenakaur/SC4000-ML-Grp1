...
3. /data/processed/feature_correlation_main_lgb1.csv (source: code1.py)
4. /data/raw/sample_submission.csv (source: data/raw/)

Output Files:
1. /output/bestline_submission_main_2.csv
...
corr_scores_df = pd.read_csv('/home/UG/aarushi003/SC4000_Final/data/processed/feature_correlation_main_lgb1.csv')

threshold = 70 # best performing threshold 
featuresFM = []
...
# Assuming oof is the out-of-fold predictions and target is the true target values
squared_errors = (oof - target) ** 2

# Create a DataFrame to store squared errors along with their indices for sorting
error_df = pd.DataFrame({
    'index': np.arange(len(oof)),  # Row indices
    'squared_error': squared_errors,
    'target': target,
    'prediction': oof
})

# Sort the errors in descending order to find the highest contributors
error_df = error_df.sort_values(by='squared_error', ascending=False)

error_df.head(5)

threshold = [-27,-26,-25,-24,-23]
for thre in threshold:
    oof = [-33.20948 if x < thre else x for x in oof]
...
sample_submission.to_csv('/home/UG/aarushi003/SC4000_Final/output/bestline_submission_main_2.csv', index=False)
...
# # Select the top 300 rows with the highest squared errors
# top_300_error_df = error_df.head(300)

# # Plot the squared errors for the top 300 rows
# plt.figure(figsize=(10, 6))
# plt.bar(top_300_error_df['index'], top_300_error_df['squared_error'], color='skyblue')
# plt.xlabel('Index of Rows')
# plt.ylabel('Squared Error')
# plt.title('Top 300 Squared Error Contributions to RMSE')
# plt.show()
